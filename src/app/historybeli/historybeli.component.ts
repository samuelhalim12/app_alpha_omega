...
@Component({
  selector: 'app-historybeli',
  templateUrl: './historybeli.component.html',
  styleUrls: ['./historybeli.component.scss'],
})
export class HistorybeliComponent implements OnInit {
  ...
  masterBarangList(event?) {
    this.as.historyBeliList().subscribe(
      (data) => {
  ...
  async openModalFilter() {
    ...
    const modal = await this.modalController.create({
      component: ModalfilterhistorybeliComponent,
      componentProps: {
    ...
    modal.onDidDismiss()
      .then(data1 => {
        ...
        this.as.filterHistoryBeli(format(startDate, "yyyy-MM-dd"), format(endDate, "yyyy-MM-dd")).subscribe(
          (data) => {
            this.masterBarang = data;
  ...
  async searchFunction() {
    ...
    // console.log(event);
    this.as.searchHistoryBeli(this.searchText).subscribe(
      (data) => {
        this.masterBarang = data;
        this.masterBarangPagination = data;
      }
